...
Measured 9y walk-forward at 10 bps: momentum-only CAGR 20.1% / Sharpe 1.08 / MaxDD -24.9% → **60/40 blend CAGR 15.1% / Sharpe 1.22 / MaxDD -15.9%**, Calmar 0.81 → 0.95, sleeve correlation 0.15, blended invested capital 0.57 → 0.69. Sharpe rises monotonically as capital shifts toward the diversifier (1.08 at 100/0, 1.15 at 80/20, 1.22 at 60/40, 1.26 at 50/50), so the split is a deliberate risk/return choice, not an optimum — 60/40 keeps most of the CAGR while taking most of the drawdown reduction. It buys risk-adjusted return and drawdown with CAGR — it is not a return improvement. Reproduce with `python main/backtest_two_sleeve.py --years 9 --cost-bps 10 --sweep`.

**The diversifier universe is defined by its exclusions.** Anything with structural equity beta is left out, because it stops diversifying exactly when it is needed. Measured 3y daily correlation to SPY: HYG 0.75, EMB 0.60, VNQ 0.52, LQD 0.37 — versus TLT 0.15, DBC 0.11, IEF 0.09, SHY 0.06, UUP -0.10. So no REITs, no high yield, no EM debt, no IG credit, and no SPY/QQQ/VEA/EEM. This is worth roughly 0.05 Sharpe and 2.4 points of drawdown on the blend: adding VNQ back moves it 1.18 → 1.15, adding credit 1.18 → 1.14. The earlier 5-ETF list (which included VNQ) was the old `backtest_cross_asset.py` universe minus its equity ETFs rather than a considered choice, and it put a 0.52-correlation REIT fund at half the sleeve.
