...
>
> **2026-08-19 two-sleeve capital split implemented and deployed.** The roadmap's own conclusion - *"Defensive allocation must happen at the capital-split level (two sleeves), not inside the stock ranking"* - was documented but never built: `main/backtest_cross_asset.py` had no deploy path. It now does (`main/deploy_sleeves.py`, `src/strategies/sleeves.py`), wired into the existing Friday workflow. Split 60/40, diversifier universe `TLT, IEF, SHY, TIP, GLD, SLV, DBC, UUP` top-3. The universe is defined by its EXCLUSIONS: every asset with structural equity beta is left out, because it stops diversifying exactly when it is needed. Measured 3y correlation to SPY - HYG 0.75, EMB 0.60, VNQ 0.52, LQD 0.37 vs TLT 0.15, DBC 0.11, IEF 0.09, SHY 0.06, UUP -0.10 - so no REITs, credit, EM debt, or SPY/QQQ/VEA/EEM. Worth ~0.05 Sharpe and ~2.4pts of drawdown: adding VNQ back moves the blend 1.18 -> 1.15, adding credit 1.18 -> 1.14. (An earlier iteration used the old cross-asset list minus equities, which left a 0.52-correlation REIT fund at half the sleeve.) Measured 9y walk-forward at 10 bps: momentum-only CAGR 20.1% / Sharpe 1.08 / MaxDD -24.9% -> 60/40 blend CAGR 15.1% / Sharpe 1.22 / MaxDD -15.9% / Calmar 0.95; sleeve correlation 0.15; blended invested capital 0.57 -> 0.69. Sharpe rises monotonically toward the diversifier (1.08 at 100/0 -> 1.26 at 50/50), so the split is a risk/return choice, not an optimum. Robustness: blend Sharpe stays 1.14-1.22 across every top_k/weight_cap tried, and the per-year record shows the sleeve earning its keep in the down years (2018 -16.7% vs -18.7%, 2022 -1.1% vs -4.6%) while lagging in strong bull years - the correct signature. Two alternatives were tested and REJECTED: widening `top_k` (Sharpe 1.01 at k=10 -> 0.79 at k=40, pure alpha dilution) and a GICS sector cap (k=10/cap-3: Sharpe 1.01->1.03, MaxDD -25.0%->-23.7%, but +33% turnover and it would import today's-GICS-membership lookahead into the signal path - a certain cost for an uncertain gain). Two latent bugs were fixed in passing: (a) `top_k * weight_cap < 1` makes `inverse_vol_weights` return a book summing to less than 1 while `realized_portfolio_vol` normalizes before estimating, so vol targeting silently undershoots (measured: a top-2 sleeve at the inherited cap 0.35 lands at gross 0.505 instead of 0.839) - now rejected by `validate_sleeves`; (b) the live deployer seeded rank-buffer incumbents from *all* account positions, which under two sleeves would feed ETFs into the stock selector - now scoped per sleeve.
